...
const RELOAD_DEBOUNCE_DELAY = config.debounceDelay;
...
class ContentScriptHotReloadClient {
  ...
  connect() {
    ...
    try {
      ...
      this.ws.onclose = (event) => {
        console.log(`[HotReload] Content-script WebSocket连接关闭: ${event.code} ${event.reason}`);
        this.isConnected = false;
      ...
      console.error('[HotReload] Content-script创建WebSocket连接失败:', error);
      this.scheduleReconnect();
    }
  }
  ...
  handleReloadRequest() {
    ...
    // 防抖处理
    if (now - this.lastNotifyTime < config.debounceDelay) {
        ...
        if (chrome.runtime && chrome.runtime.id) {
          chrome.runtime.sendMessage({ type: 'request-reload' }, () => {
            if (chrome.runtime.lastError) {
              console.warn('[HotReload] Content-script发送重载请求失败:', chrome.runtime.lastError);
            }
          });
        } else {
          performReload();
  ...
  scheduleReconnect() {
    if (this.reconnectAttempts >= this.maxReconnectAttempts) {
      console.error(`[HotReload] Content-script达到最大重连次数 (${this.maxReconnectAttempts})，停止重连`);
      return;
    }
    ...
    console.log(`[HotReload] Content-script第 ${this.reconnectAttempts} 次重连，${delay}ms 后重试`);
...
document.addEventListener('visibilitychange', () => {
  if (document.hidden && contentHotReloadClient) {
    contentHotReloadClient.disconnect();
  } else if (!document.hidden && contentHotReloadClient && !contentHotReloadClient.isConnected) {
