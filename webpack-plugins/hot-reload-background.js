...
class ChromeHotReloadClient {
  ...
  connect() {
    ...
    try {
      ...
      this.ws.onclose = (event) => {
        console.log(`[HotReload] WebSocket连接关闭: ${event.code} ${event.reason}`);
        this.isConnected = false;
  ...
  handleReload() {
    ...
    // 防抖处理
    if (now - this.lastReloadTime < config.debounceDelay) {
      return;
    }
  ...
  scheduleReconnect() {
    ...
    console.log(`[HotReload] 第 ${this.reconnectAttempts} 次重连，${delay}ms 后重试`);
